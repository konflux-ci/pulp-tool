...
- `upload-build` command (`upload` remains a compatibility alias): ORAS publish of `pulp_results.json` via **`--oci-storage`** (Konflux `ociStorage`) or `cli.oci_storage`; with `--artifact-results`, Tekton files get OCI image URL (without digest) and `sha256:` digest matching import-to-quay `PULP-IMAGE_*` results ([e7f834f](https://github.com/konflux-ci/pulp-tool/commit/e7f834f))
- `pull --oci-storage` for side-tag ORAS publish (flag overrides transfer config) ([e7f834f](https://github.com/konflux-ci/pulp-tool/commit/e7f834f))
- `pull --side-tag` (with `--transfer-dest`): extra ROK RPM repo/distribution, versioned `pulp_results.json` merge, ORAS push, `--artifact-results` Tekton outputs, and optional `--snapshot-path` (`pulpResultsOciManifest`) for trusted-artifact release workspaces ([e7f834f](https://github.com/konflux-ci/pulp-tool/commit/e7f834f))
- `pulp-tool-container` image: ORAS CLI, `jq`, and Konflux `select-oci-auth` for registry auth (`oras --registry-config`); e2e ORAS tests when `E2E_OCI_STORAGE` is set ([e7f834f](https://github.com/konflux-ci/pulp-tool/commit/e7f834f))
